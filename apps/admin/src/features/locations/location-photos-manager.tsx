...
import {
  useCreateLocationPhoto,
  useDeleteLocationPhoto,
  useLocationPhotos,
  usePatchLocationPhoto,
} from '@/hooks/use-location-photos';
import type { LocationPhoto } from '@/lib/location-photos-api';
import { UploadError, uploadFile } from '@/lib/uploads-api';

/**
 * Per-location photos / hero carousel manager.
 *
 * Upload flow per file:
 *   1. uploadFile('photo', f) → R2 direct PUT (tenant photo sign path is fine
 *      for the bytes — the row is what scopes it to this location).
 *   2. createLocationPhoto(locationId, { url }) → DB row.
 ...
 */
export function LocationPhotosManager({ locationId }: { locationId: string }) {
  const { t } = useTranslation();
  const photosQuery = useLocationPhotos(locationId);
  const create = useCreateLocationPhoto(locationId);
  const patch = usePatchLocationPhoto(locationId);
  const del = useDeleteLocationPhoto(locationId);
  ...
  const onPickFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Snapshot into a real array BEFORE resetting the input — clearing
    // e.target.value empties the live FileList reference in Chromium.
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-ink-400">
        {t('tenantAdmin.locations.photosHint', {
          defaultValue:
            'Select one or more images. The hero photo appears on Main feed cards; the rest fill the location page carousel.',
        })}
      </p>
